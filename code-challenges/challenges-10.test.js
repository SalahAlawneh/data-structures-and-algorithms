...
const count = (target, input) => {
  let repeatedNumber = Array.prototype.concat.apply([], input).filter(e => e === target)
...
const totalSum = (input) => {
  let makeThemOneArray = Array.prototype.concat.apply([], input);
  return makeThemOneArray.reduce((a, b, c) => {
    return a = a + b
  }, 0)
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.reduce((a, b) => {
    a.push(b.filter(e => typeof e === 'number' && e % 5 === 0).map(e2 => Math.pow(2, e2)));
    return a;
  }, []);
};
...
let findMaleAndFemale = (data) => {
  ...
  return arraOfNames.map(e => e.name).join(" and ");
};
