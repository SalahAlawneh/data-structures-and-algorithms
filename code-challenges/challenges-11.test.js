...
const validatePin = (pin) => pin.toString().match(/^[0-9][0-9][0-9][0-9]$/) ? true : false;
...
const validateEmail = (email) => (email.match(/^\w+(.\w+)@\w+\.(net|com|org)$/)) ? true : false;
...
const validatePhoneNumber = (phoneNumber) => (phoneNumber.match(/^([0-9]{3}|\([0-9]{3}\))[^0-9]?([0-9]{3})[^0-9]?([0-9]{4})$/)) ? true : false;
