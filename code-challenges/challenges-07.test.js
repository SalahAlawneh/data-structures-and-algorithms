...
const addOne = (arr) => {
  let incrementedBy1 = arr.map(element => element + 1);
  return incrementedBy1;
}
...
const addQuestion = (arr) => {
  let addedQuestionMarkCharacter = arr.map(element => element + "?");
  return addedQuestionMarkCharacter;
}
...
const forLoopTwoToThe = (arr) => {
  ...
  for (const iterator of arr) {
    newArray.push(Math.pow(2, iterator));
  }
  return newArray;
};
