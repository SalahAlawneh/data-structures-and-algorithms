...
const containsW = (str) => {
  let regularEpression = /w/g;
  return regularEpression.test(str);
};
...
const isNum = (input) => {
  let regularEpression = /[0-9]/g;
  return regularEpression.test(input);
};
...
const isCapitalized = (str) => {
  let regex = /\b[A-Z].*?\b/g;
  let arrayOfMatches = str.match(regex);
...
const citiesAtoJ = (arr) => {
  ...
  let newArray = [];
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].match(regex)) newArray.push(arr[i]);
  }
  return newArray;
};
